...
#                          facetVariables = character(0),
#                          xLab = "",
#                          yLabs = c("intensity","log duration","pitch (semitones)"),
#                          use_repel = FALSE,
#                          point_size = 3,
#                          errorbar_width = 0.15,
#                          xnudge = 0.25,
...
#   if (!require(dplyr, quietly = TRUE)) stop("Package 'dplyr' is required.")
#   if (!require(ggplot2, quietly = TRUE)) stop("Package 'ggplot2' is required.")
#   if (!require(patchwork, quietly = TRUE)) stop("Package 'patchwork' is required.")
#   
#   # Load ggrepel only if requested
#   repel_available <- FALSE
#   if (isTRUE(use_repel)) {
...
plotAcoustics <- function(data,
  ...
  # ---- helpers ----
  # build facet layer from 0, 1, or 2 variables
  ...
  # axis label with Delta using plotmath, constructed from string
  delta_label <- function(lbl_string) {
    # returns an expression like: expression(Delta~"pitch (semitones)")
    bquote(Delta ~ .(lbl_string))
  }
  
  # tidy eval handle for x var
  x_quo <- rlang::enquo(xVar)
  
  # base data: drop NAs in x and facets (y handled per-plot)
  base_data <- data |>
    dplyr::filter(
      !is.na(!!x_quo),
      dplyr::if_all(dplyr::all_of(facetVariables), ~ !is.na(.x))
  ...
  # one panel builder (for a given y column name and label text)
  make_panel <- function(y_name, ylab_text) {
    # filter NA for current y
    df <- base_data |>
      dplyr::filter(!is.na(.data[[y_name]]))
    
    # summary for labels (mean and n) by x and facets
    lab_df <- df |>
    ...
    # add labels: either repel (with separate data) or plain text with nudge
    if (isTRUE(use_repel) && repel_available) {
      p <- p +
      ...
      p <- p +
        ggplot2::geom_text(
          ...
          ggplot2::aes(x = !!x_quo, y = m, label = label),
          inherit.aes = FALSE,
          hjust = 0.5,                              # center the two lines relative to each other
          vjust = label_vjust,
          size = 3,
          position = ggplot2::position_nudge(x = xnudge)  # shift block to the right of the mean
        )
    }
    
    # add facets, labels, theme
    if (!is.null(facet_layer)) p <- p + facet_layer
  ...
  p1 <- make_panel(yVar[1], yLabs[1])  # intensity
  p2 <- make_panel(yVar[2], yLabs[2])  # duration
  p3 <- make_panel(yVar[3], yLabs[3])  # pitch
  
  # ---- combine with patchwork and add a shared title ----
  combined <- (p1 | p2 | p3) +
...
plotAcoustics2 <- function(data,
  ...
  make_panel <- function(y_name, ylab_text, show_x = FALSE) {
    df <- base_data |>
      dplyr::filter(!is.na(.data[[y_name]]))
  ...
  # top two panels hide x-axis; bottom shows it
  p1 <- make_panel(yVar[1], yLabs[1], show_x = FALSE) # intensity
  p2 <- make_panel(yVar[2], yLabs[2], show_x = FALSE) # duration
  p3 <- make_panel(yVar[3], yLabs[3], show_x = TRUE)  # pitch (shows x-axis)
